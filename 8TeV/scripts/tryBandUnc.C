...
//    hGJMass_mc->Scale(1.42*0.98*0.95);
    hGJMass_mc->Scale(1.35*0.98*0.95);
    hGJMass_mc->Add(hGJMass_mc2,1.3*0.98*0.95);
    hGJMass_mc->Add(hGJMass_mc3,0.98*0.95);
    hGJMass_mc->SetLineWidth(2);

    Double_t minXFit = 561.0 ;
	...
	h_err_bkg->SetBinError(k, sqrt( pow(h_err_bkg->GetBinError(k),2) + 0.01*(h_err_bkg->GetBinContent(k)**2) + 0.002*(h_err_bkg->GetBinContent(k)**2) ));
...
//    TLegend *leg = new TLegend(0.2,0.1,0.5,0.4);
    ...
    leg->AddEntry(fit,"Backgraound from Fit","L");
 //   leg->AddEntry(h_tmp,"1#sigma Fit Error ","F");
    leg->AddEntry(hGJMass_mc,"Backround from MC","L");
    leg->AddEntry(h_err_bkg,"Background Uncertainity ","F");
    leg->Draw("same");

//    TPaveText *fit_para = new TPaveText(0.65,.6,0.95,0.93,"NDC");
    TPaveText *fit_para = new TPaveText(0.15,0.1,0.4,0.35,"NDC");
    //    fit_para->AddText("#sqrt{s} = 8 TeV");
    fit_para->AddText("CMS Preliminary  ");
    fit_para->AddText("#int Ldt=19.74fb^{-1}");
    fit_para->AddText("#sqrt{s} = 8 TeV");
...
//    fit_para->AddText(Form("#chi^{2}/ndf : %2.2f/%2.2f",func_FCN[0],func_NDF[0]));
    ...
    TPaveText *pt_qstar1000 = new TPaveText(0.2483145,0.830821,0.3770044,0.9317112,"NDC");
    ...
    TPaveText *pt_qstar1500 = new TPaveText(0.4066144,0.6864095,0.5273324,0.7872997,"NDC");
    ...
    TPaveText *pt_qstarf2500 = new TPaveText(0.70,0.3,0.88,0.40,"NDC");
