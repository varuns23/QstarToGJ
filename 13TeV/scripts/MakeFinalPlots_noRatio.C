...
//    double x_max = 1600.0;
	...
	hBkg3->Scale(scaleMC);
	if(rebin)hBkg3->Rebin(rebinBy);
	hBkg3->GetXaxis()->SetRangeUser(x_min,x_max);

	hData->SetStats(0); 
	hData->SetMarkerStyle(8); 
	hData->SetMarkerColor(kBlack); 
	hData->SetMarkerSize(1.0);  //0.8
	hData->SetLineColor(kBlack);
	if(rebin)hData->Rebin(rebinBy);
	...
	TPaveText *cms = new TPaveText(0.2,0.9,0.8,0.95,"blNDC");
	...
	TCanvas* c1 = new TCanvas("c1","",80,20,800,700);
	c1->SetLogy(); c1->SetFillColor(0); c1->SetFrameBorderMode(0); c1->SetBorderMode(0);
